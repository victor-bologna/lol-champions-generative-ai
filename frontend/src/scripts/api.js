const routes = {
  championsAsk: "http://sdw24-victor.sa-east-1.elasticbeanstalk.com/champions/{id}/ask",
  champions: "http://sdw24-victor.sa-east-1.elasticbeanstalk.com/champions"
};
...
const apiService = {
  ...
  async postAskChampions (id, message) {
    const route = routes.championsAsk.replace("{id}", id);
    const options = {
      method: "POST",
      headers: {
        "accept": "*/*",
...
async function fetchAskChampion() {
  ...
  const id = state.values.idSelected;
